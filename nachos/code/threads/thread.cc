...
Thread::Thread(char* threadName,int j, int w)
{
    name = threadName;
    stackTop = NULL;
    stack = NULL;
    ...
    Joined = false;
    child_finish =false;
    child_called = new Condition("treadName");
    child_end = new Condition("treadName");
...
Thread::Finish ()
{
    ...
    if(join == 1){
      joinLock->Acquire();
      while(!Joined){
          printf("child Thread keeps waiting to be called\n");
          child_called->Wait(joinLock);
        }
        child_end->Signal(joinLock);
        child_finish = true;
        printf("child Thread has been called and finished\n");
        joinLock->Release();
     }
